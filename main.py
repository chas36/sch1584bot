...
@bot.callback_query_handler(func=lambda call: call.data.startswith("class_") or
                            call.data == "choose_class")
  ...
  for student in data:
    if student['Класс'] == selected_class:
      ...
      if len(name_parts) >= 3:
        ...
        key = " ".join(
            name_parts
        )  # Простое соединение частей, или другая логика обработки
  ...
  for student in data:
    if student['Класс'] == selected_class:
      ...
      if len(name_parts) >= 3:
        ...
        key = " ".join(
            name_parts
        )  # Используйте адаптированный ключ для меньшего количества частей

      # Если есть совпадения, используем полное имя без отчества
      if name_counts[key] > 1:
        name_to_use = " ".join(
            name_parts[:2])  # Использовать имя и фамилию без отчества
      else:
...
def my_classes(message):
  ...
  if selected_classes:
    bot.send_message(chat_id,
                     f"Ваши выбранные классы: {', '.join(selected_classes)}")
  else:
    bot.send_message(
        chat_id,
        """Вы еще не выбрали ни одного класса. Напишите "Выбрать класс".""")
...
def handle_confirm_selection(call):
  ...
  if selected_classes:
    ...
    bot.send_message(call.message.chat.id,
                     "Ни один класс не выбран. Выберите класс снова.")
...
def choose_class(message):
  ...
  if selected_classes:
    ...
    classes_for_parallel = sorted(
        set(class_name for class_name in class_column
            if class_name.startswith(selected_parallel)))

    # Создаем кнопки для выбора класса
    class_buttons = [
        types.InlineKeyboardButton(class_name,
...
def handle_text(message):
  ...
  if user_message in unique_classes:
    ...
    bot.send_message(
        chat_id,
        """Пожалуйста, выберите свой класс с написав "Выбрать класс".""")
...
def setup_schedule():
  start_time = datetime.now().replace(hour=5,
                                      minute=0,
                                      second=0,
                                      microsecond=0)
  end_time = start_time.replace(hour=7)
  current_time = start_time
  intervals = [(6, 30), (7, 15)]  # (Час окончания, интервал)

  for end_hour, interval in intervals:
    while current_time.hour < end_hour:
      schedule.every().day.at(
          current_time.strftime("%H:%M")).do(send_reminders)
...
def show_classes_to_user(chat_id, user_classes):
  ...
  all_lists_button = types.InlineKeyboardButton("Отправить все списки",
                                                callback_data="send_all_lists")
...
def show_students_for_class(chat_id, selected_class):
  ...
  buttons = [
      types.InlineKeyboardButton(f"{name}",
                                 callback_data=f"absent_{student_id}")
  ...
  all_present_button = types.InlineKeyboardButton("Все присутствуют",
                                                  callback_data="all_present")
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("select_class_"))
def handle_select_class(call):
  user_id = call.from_user.id
  class_name = call.data.split("select_class_")[1]
  current_class_selection[
      user_id] = class_name  # Сохраняем текущий выбранный класс
...
def handle_student_absent(call):
  ...
  student_name = get_student_name_by_id(
      student_id)  # Получите имя ученика по ID
  class_name = get_class_by_student_id(
      student_id)  # Получите класс ученика по ID
  ...
  for pair in reason_pairs:
    row_buttons = [
        types.InlineKeyboardButton(
            reason,
            callback_data=f"reason_{student_id}_{reason.replace(' ', '_')}")
        for reason in pair
    ]
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("come_to_lesson"))
def handle_come_to_lesson(call):
  print("handle_come_to_lesson")
  _, student_id = call.data.rsplit(
      "_", 1)  # Используйте rsplit для корректной работы с student_id
...
def handle_lesson_selection(call):
  ...
  bot.edit_message_text(
      ...
      message_id=call.message.message_id,
      text=
      f"Отмечено отсутствие: {student_name} Придет к {lesson_number} уроку",
...
def mark_student_absent(call):
  ...
  student_name = get_student_name_by_id(
      student_id)  # Предполагаемая функция для получения имени ученика
  class_name = get_class_by_student_id(
      student_id)  # Предполагаемая функция для получения класса ученика
...
@bot.callback_query_handler(
    func=lambda call: call.data == "finish_absence_list")
def handle_finish_absence_list(call):
  ...
  if selected_classes:
    if len(selected_classes) > 1:
      ...
      selected_class = selected_classes[
          0]  # Прямое использование первого и единственного класса
      if selected_class in absences and absences[selected_class]:
        send_absence_list_to_recipient(selected_class,
                                       absences[selected_class], user_id)
        bot.send_message(
            call.message.chat.id,
        ...
        bot.send_message(call.message.chat.id,
                         "Список отсутствующих учеников пуст.")
  else:
...
def send_absence_list_to_recipient(selected_class, absence_list, sender_id):
  ...
  for student in sorted_absence_list:
    ...
    reason_short = reason_mapping.get(
        reason, reason)  # Получаем сокращение или оставляем как есть
  ...
  all_recipients = set(
      RECIPIENT_CHAT_ID)  # Преобразуем в множество для уникальности
